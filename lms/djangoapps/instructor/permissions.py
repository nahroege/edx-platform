...
from lms.djangoapps.courseware.rules import HasAccessRule, HasRoleRule
...
perms[VIEW_ISSUED_CERTIFICATES] = HasAccessRule('staff') | HasRoleRule('data_researcher')
perms[CAN_RESEARCH] = HasRoleRule('data_researcher')
perms[CAN_ENROLL] = HasAccessRule('staff')
perms[CAN_BETATEST] = HasAccessRule('instructor')
perms[ENROLLMENT_REPORT] = HasAccessRule('staff') | HasRoleRule('data_researcher')
perms[VIEW_COUPONS] = HasAccessRule('staff') | HasRoleRule('data_researcher')
perms[EXAM_RESULTS] = HasAccessRule('staff')
perms[OVERRIDE_GRADES] = HasAccessRule('staff')
perms[SHOW_TASKS] = HasAccessRule('staff') | HasRoleRule('data_researcher')
perms[EMAIL] = HasAccessRule('staff')
perms[RESCORE_EXAMS] = HasAccessRule('instructor')
perms[VIEW_REGISTRATION] = HasAccessRule('staff')
perms[VIEW_DASHBOARD] = HasAccessRule('staff') | HasRoleRule('data_researcher')
